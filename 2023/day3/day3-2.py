...
def getGears(token: str, lines: list) -> list:
    if not isinstance(token.token, int): return []
    ...
    for neighbour in token.neighbours:
        ...
        if lines[neighbour.y][neighbour.x] == '*':
            gears.append([neighbour, token.token])
...
with open('2023/day3/input.txt', 'r') as f:
    ...
    for lineTokens in tokens:
        for token in lineTokens:
            gears = getGears(token, lines)
            if len(gears) > 0:
                for gear in gears:
                    for token1 in tokens[gear[0].y]:
                        if token1.token == '*' and token1.pos.x == gear[0].x:
                            token1.gears.append(gear[1])
                
    for lineTokens in tokens:
        for token in lineTokens:
            if token.token == '*':
                if len(token.gears) == 2:
                    sum_ += token.gears[0] * token.gears[1]
